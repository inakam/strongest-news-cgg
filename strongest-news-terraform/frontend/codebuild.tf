...
resource "aws_codebuild_project" "cgg-framework" {
  ...
  environment {
    ...
    environment_variable {
      ...
      type  = "PLAINTEXT"
      value = aws_ecr_repository.cgg-framework-react.name
    }
    environment_variable {
      name  = "IMAGE_TAG"
      type  = "PLAINTEXT"
    ...
    environment_variable {
      name  = "REACT_APP_BACKEND_API_ENDPOINT"
      type  = "PLAINTEXT"
      value = "http://${var.api_endpoint}"
    }
  }
  ...
  source {
    ...
    type                = "CODECOMMIT"
    buildspec           = "codebuild/frontend/buildspec.yml"
  }
}
